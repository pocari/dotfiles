...
call denite#custom#source('file/old', 'matchers', ['matcher_cpsm', 'matcher_project_files'])
call denite#custom#source('file/rec', 'matchers', ['matcher_cpsm'])
call denite#custom#source('directory_rec', 'matchers', ['matcher_cpsm'])

" file/recはag, directory_recの代わりにdirectory_oldを使うようにしたので一旦コメント
" if executable('files')
"   " https://github.com/mattn/files
"   call denite#custom#var('file/rec', 'command', ['files', '-a'])
